...
switch ($action) {
    case 'edit':
        render_edit_form($tbPref, $tableName, $id);
        break;
    case 'add':
        render_add_form($tbPref, $tableName);
        break;
    case 'list':
    default:
        render_list_view($tbPref, $tableName);
        break;
}
...
function render_list_view(string $tbPref, string $tableName): void
{
    ...
    start_form(false, false, 'add-form', 'ksf-pd-add-form');
    echo '<input type="hidden" name="action" value="add">';
    submit_center('AddNew', _('Add New Mapping'), true, _('Add New Mapping'));
    end_form();
...
function render_add_form(string $tbPref, string $tableName): void
{
    ...
    echo '<td>' . bank_accounts_list('bank_account', '', false, false) . '</td>';
    end_row();
    end_table(1);
    submit_center('Save', _('Save Mapping'));
    echo ' <a href="controller.php?action=list">' . _('Cancel') . '</a>';
    end_form();
}

function render_edit_form(string $tbPref, string $tableName, ?int $term): void
{
    $row = null;
    if ($term) {
        $editQb = new \ksfraser\PaymentDestinations\QueryBuilder\QueryBuilder($tableName);
        $editQb->select()->where('payment_term', $term);
        $row = db_fetch(db_query($editQb->toFaSql(), 'load edit row'));
    }

    echo '<h3>' . _('Edit Mapping') . '</h3>';
    echo '<form method="post" action="controller.php">';
    echo '<input type="hidden" name="action" value="save">';
    echo '<input type="hidden" name="payment_term" value="' . ($row['payment_term'] ?? $term) . '">';
    start_table(TABLESTYLE2, 'width=40%');
    $th = [_('Payment Term'), _('Bank Account')];
    table_header($th);
    start_row();
    $selectedTerm = $row['payment_term'] ?? $term;
    echo '<td>' . sale_payment_list('payment_term', $selectedTerm, false, true) . '</td>';
    $selectedBank = $row['bank_account'] ?? '';
    echo '<td>' . bank_accounts_list('bank_account', $selectedBank, false, false) . '</td>';
    end_row();
    end_table(1);
    submit_center('Update', _('Update Mapping'));
    echo ' <a href="controller.php?action=list">' . _('Cancel') . '</a>';
    end_form();
}
